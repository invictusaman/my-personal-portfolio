...
Only the rows that changed since the last run get copied — and after a successful copy, the JSON watermark is updated to the current timestamp. Clean, efficient, and no full table scans.
...
| ---------------------- | ------------------- | ----------------------------------- |
| Normal daily run       | _(empty)_           | Uses watermark — incremental only   |
...
// Inside the loop, each item() exposes:
  item().table_name          → "dbo.Orders" # [!code --]
  item().last_updated_date   → "2025-01-28T14:30:00Z" # [!code --]
```
...
<Image
  src="/blog/data-ingestion-project/Flow-of-Execution-Azure-Ingestion.png"
  width="1536"
  height="1024"
